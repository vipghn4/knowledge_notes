...
- [OOP features of Rust](#oop-features-of-rust)
  ...
  - [Using trait objects allowing for values of different types](#using-trait-objects-allowing-for-values-of-different-types)
...
**Enforcement of encapsulation**. We can use the `pub` keyword to decide which modules, types, functions, and methods in our code should be public

>**NOTE**. By default everything else is private

* *Public struct*. Indicate that other code can use the struct, e.g.
    ...
    >**NOTE**. The fields within the struct remain private
...
* *Benefits of encapsulation*. 
    * We can easily change aspects, e.g. the data structure, in the future
    * The option to use `pub` or not for different parts of code enables encapsulation of implementation details

### Inheritance as a type system and as code sharing
**Inheritance**. A mechanism, where an object can inherit from another object’s definition

$\to$ The object gains the parent object’s data and behavior without us having to define them again
* *Supports for inheritance of Rust*. There is no way to define a struct that inheriting the parent struct’s fields and method implementations
    * *Solution*. If we need inheritance in the programming toolbox
        
        $\to$ We can use other solutions in Rust, depending on the reason for reaching for inheritance in the first place
* *Needs for inheritance*. We choose inheritance for two main reasons
    * *Code reuse*. We can implement particular behavior for one type, and inheritance enables us to reuse that implementation for a different type
        * *Solution in Rust*. Use default trait method implementations
            * *Idea*. Any type implementing a trait would have the trait's methods available on it without any further code
                
                $\to$ This is similar to a parent class having an implementation of a method and an inheriting child class also having the implementation of the method
            * *Implementation overriding*. We can override the default implementation of the inherited methods when implementing a trait
                
                $\to$ This is similar to a child class overriding the implementation of a method inherited from a parent class
    * *Polymorphism*. Enable a child type to be used in the same places as the parent type
        * *Difference from inheritance*. Polymorphism is a more general concept referring to code that can work with data of multiple types
            
            $\to$ For inheritance, those types are generally subclasses
        * *Solution in Rust*. Use generics to abstract over different possible types and trait bounds to impose constraints on what those types must provide
            
            $\to$ This is sometimes called bounded parametric polymorphism

**Inheritance in recent programming designs**. Inheritance has recently fallen out of favor as a programming design solution in many programming languages
* *Explain*. It is often at risk of sharing more code than necessary, i.e. subclasses should not always share all characteristics of their parent class
    * This can make a program’s design less flexible
    * This introduces the possibility of calling methods on subclasses that do not make sense or that cause errors 
        * *Explain*. The methods do not apply to the subclass
    * Some languages only allow a subclass to inherit from one class, further restricting the flexibility of a program’s design
* *Consequence*. Rust takes a different approach, using trait objects instead of inheritance

## Using trait objects allowing for values of different types
