...
* *Mechanism*.
    ...
    2. If the word is in the cache, it is delivered to the processor
    3. Otherwise, a block of memory consisting of some fixed number of words is read into the cache
        
        $\to$ The word is then delivered to the processor
...
2. If the word is in the cache, it is delivered to the process
3. Otherwise, the block containing the word is loaded into the cache, and the word is delivered to the processor
...
* *Mechanism*.
    * When a cache hit occurs, the data and address buffers are disabled
        ...
        $\to$ Communication is only between processor and cache, without system bus traffic
    * When a cache miss occurs, the desired address is loaded onto the system bus
        
        $\to$ The data are returned through the data buffer to both the cache and the processor
